...
export default async function GetStarted({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <div className="max-h-screen w-screen bg-background flex flex-col items-center gap-10">
        <header className="gap-2 p-5 text-center flex flex-col items-center w-full max-w-md relative">
            ...
            {/*title*/}
            <p className="text-3xl font-bold text-bright-purple text-center">{t("title")}</p>
            {/*description*/}
        ...
        {/*text name*/}        
        <div className="flex flex-col w-full items-center p-5 pt-0 -mt-4">
          <label htmlFor="name" className="text-2xl font-bold text-bright-purple">{t("name")}</label>
          <input type="text" id="name" placeholder={t("username")} className="w-9/10 max-w-md p-4 rounded-full border-4 border-bright-purple shadow-card mt-4 text-lg font-bold" />
        </div>
